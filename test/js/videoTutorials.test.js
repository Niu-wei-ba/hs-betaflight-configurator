...
describe("video tutorial tab availability", () => {
    it("remains allowed after a flight controller connects", async () => {
        const { createGui } = await import("../../src/js/gui.js");
        const gui = createGui();

        expect(gui.defaultAllowedTabsWhenDisconnected).toContain("video_tutorials");
        expect(gui.defaultAllowedFCTabsWhenConnected).toContain("video_tutorials");
    });
});
