...
function buildApiUrl(path = "") {
    const configuredBaseUrl = String(import.meta.env.VITE_BUILD_API_BASE_URL || "").replace(/\/+$/, "");
    const currentOrigin = typeof window !== "undefined" && window.location?.origin ? window.location.origin : "";
    return `${configuredBaseUrl || currentOrigin}/api${ensureLeadingSlash(path)}`;
}
...
export function openVideoTutorials(categoryId = null) {
    ...
    import("./tab_switch.js").then(({ switchTab }) => {
        switchTab("video_tutorials", { mode: "shared" });
